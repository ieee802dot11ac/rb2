...
_STLP_BEGIN_NAMESPACE
    ...
    public:
        ...
        ~_Vector_base() {
            if (_M_start != 0)
                _M_end_of_storage.deallocate(
                    _M_start, _M_end_of_storage._M_data - _M_start
                );
        }
...
#define _VECTOR_IMPL vector
    ...
    protected:
        ...
        void _M_insert_overflow_aux(
            ...
            if (_M_is_inside(__x)) {
                ...
                _M_insert_overflow_aux(
                    __pos, __x_copy, __false_type(), __fill_len, __atend
                );
                return;
            }
        ...
        void _M_insert_overflow(
            pointer __pos,
            const _Tp &__x,
            const __false_type & /*_TrivialCpy*/,
            size_type __fill_len,
            bool __atend = false
        ) {
    ...
    public:
        ...
        size_type max_size() const {
            ...
            return (__alloc_max_size < __vector_max_size) ? __alloc_max_size
                                                          : __vector_max_size;
        }
        ...
        _VECTOR_IMPL(
            ...
            this->_M_finish =
                __uninitialized_fill_n(this->_M_start, __n, __val, _PODType());
        }

#if defined(_STLP_DONT_SUP_DFLT_PARAM)
        explicit _VECTOR_IMPL(size_type __n)
            : _Vector_base<_Tp, _Alloc>(__n, allocator_type()) {
            this->_M_finish = __uninitialized_fill_n(
                this->_M_start, __n, _STLP_DEFAULT_CONSTRUCTED(_Tp), _PODType()
            );
        }
#endif /*_STLP_DONT_SUP_DFLT_PARAM*/
        ...
        _VECTOR_IMPL(const _Self &__x)
            ...
            this->_M_finish = __uninitialized_copy(
                __x.begin(), __x.end(), this->_M_start, _TrivialUCpy()
            );
        }
        ...
        void
        _M_initialize_aux(_Integer __n, _Integer __val, const __true_type & /*_IsIntegral*/) {
        ...
        void
        _M_assign_aux(_ForwardIter __first, _ForwardIter __last, const forward_iterator_tag &) {
#else
        ...
        void
        _M_assign_aux(_InputIter __first, _InputIter __last, const input_iterator_tag &) {
        ...
        template <class _Integer>
        void
        _M_assign_dispatch(_Integer __n, _Integer __val, const __true_type & /*_IsIntegral*/) {
            _M_fill_assign(__n, __val);
        }

        template <class _InputIter>
        void _M_assign_dispatch(
            _InputIter __first, _InputIter __last, const __false_type & /*_IsIntegral*/
        ) {
    ...
    private:
        void _M_fill_insert_aux(
            iterator __pos, size_type __n, const _Tp &__x, const __true_type & /*_Movable*/
        );
        void _M_fill_insert_aux(
            iterator __pos,
            size_type __n,
        ...
        void _M_range_insert_realloc(
            ...
            _STLP_TRY {
                __new_finish = __uninitialized_move(
                    this->_M_start, __pos, __new_start, _TrivialUCpy(), _Movable()
                );
                __new_finish =
                    __uninitialized_copy(__first, __last, __new_finish, _TrivialUCpy());
                __new_finish = __uninitialized_move(
                    __pos, this->_M_finish, __new_finish, _TrivialUCpy(), _Movable()
                );
            }
            _STLP_UNWIND(
                (_STLP_STD::_Destroy_Range(__new_start, __new_finish),
        ...
        void _M_range_insert_aux(
            ...
            if (__elems_after > __n) {
                __uninitialized_copy(
                    this->_M_finish - __n, this->_M_finish, this->_M_finish, _TrivialUCpy()
                );
                this->_M_finish += __n;
                __copy_backward_ptrs(
                    __pos, __old_finish - __n, __old_finish, _TrivialAss()
                );
        ...
        template <class _Integer>
        void
        _M_insert_dispatch(iterator __pos, _Integer __n, _Integer __val, const __true_type &) {
            _M_fill_insert(__pos, (size_type)__n, (_Tp)__val);
        }

        template <class _InputIterator>
        void
        _M_insert_dispatch(iterator __pos, _InputIterator __first, _InputIterator __last, const __false_type &) {
            _M_range_insert(
    ...
    private:
        template <class _InputIterator>
        void
        _M_range_insert(iterator __pos, _InputIterator __first, _InputIterator __last, const input_iterator_tag &) {
        ...
        template <class _ForwardIterator>
        void
        _M_range_insert(iterator __pos, _ForwardIterator __first, _ForwardIterator __last, const forward_iterator_tag &) {
#else /* _STLP_MEMBER_TEMPLATES */
    public:
    ...
    protected:
        ...
        iterator
        _M_erase(iterator __first, iterator __last, const __true_type & /*_Movable*/) {
        ...
        iterator
        _M_erase(iterator __first, iterator __last, const __false_type & /*_Movable*/) {
    ...
    public:
        iterator erase(iterator __pos) { return _M_erase(__pos, _Movable()); }
...
#if !defined(_STLP_DONT_SUP_DFLT_PARAM)
        void
        resize(size_type __new_size, const _Tp &__x = _STLP_DEFAULT_CONSTRUCTED(_Tp)) {
#else
        void resize(size_type __new_size, const _Tp &__x) {
#endif /*_STLP_DONT_SUP_DFLT_PARAM*/
            if (__new_size < size()) erase(begin() + __new_size, end());
...
#if defined(_STLP_MEMBER_TEMPLATES)
    template <class _ForwardIterator>
    pointer _M_allocate_and_copy(
        size_type __n, _ForwardIterator __first, _ForwardIterator __last
    )
#else /* _STLP_MEMBER_TEMPLATES */
        pointer
        _M_allocate_and_copy(size_type __n, const_pointer __first, const_pointer __last)
#endif /* _STLP_MEMBER_TEMPLATES */
    {
        pointer __result = this->_M_end_of_storage.allocate(__n);
        _STLP_TRY {
#if !defined(__MRC__) //*TY 12/17/2000 - added workaround for MrCpp. it confuses on nested
                      //try/catch block
...
#include <system/utl/StlAlloc.h>
    ...
    template <class _InputIterator>
    void
    _M_range_initialize(_InputIterator __first, _InputIterator __last, const input_iterator_tag &) {
        for (; __first != __last; ++__first)
            push_back(*__first);
    }
    ...
    template <class _ForwardIterator>
    void
    _M_range_initialize(_ForwardIterator __first, _ForwardIterator __last, const forward_iterator_tag &) {
        size_type __n = distance(__first, __last);
        this->_M_start = this->_M_end_of_storage.allocate(__n);
        this->_M_end_of_storage._M_data = this->_M_start + __n;
        this->_M_finish =
